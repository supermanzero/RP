...
define(function () {
     ...
     * dom:包裹元素id，class
     * demo:selectcity("domID")
     */
    var selectcity=function(dom,selectedArr){
        var first=$(dom).find("select").eq(0),
            md=$(dom).find("select").eq(1),
            last=$(dom).find("select").eq(2);
        var idList = {province:{},city:{},district:{}};
        $.ajax({
            type:"GET",
            ...
            success:function(msg){
                ...
                $.each(msg.list,function(key,val){
                    ...
                    html+="<option value='"+val.diming+"'>"+val.diming+"</option>";
                    idList.province[val.diming] = val.daima;
                })
                first.append(html)

                first.val(selectedArr[0]);
                md.val(selectedArr[1]);
                last.val(selectedArr[2]);
                first.change()
                md.change()
            }
        })
                ...
                success: function(msg){
                    if(idd=="province"){
                        ...
                        $.each(msg.list,function(key,val){
                            //html+="<option value="+val.daima+">"+val.diming+"</option>";
                            html+="<option value='"+val.diming+"'>"+val.diming+"</option>";
                            idList.city[val.diming] = val.daima;
                        })
                        md.append(html)
//                            for(c in msg){
...
//                                    }
                        ...
                        $.each(msg.list,function(key,val){
                            //html+="<option value="+val.daima+">"+val.diming+"</option>";
                            html+="<option value="+val.diming+">"+val.diming+"</option>";
                            idList.district[val.diming] = val.daima;
                        })
                        last.append(html)
//                            for(c in msg){
//                                var html="<option value="+msg[c][0]+">"+msg[c][1]+"</option>";
